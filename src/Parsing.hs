...
peekb (MkLexBuf _ _ (x:_)  _) = x

forward :: LexBuf t -> LexBuf t
forward (MkLexBuf n ll (x:rr) nmax) =
  MkLexBuf (n+1) (x:ll) rr (nmax `max` (n+1))
...
commit (MkLexBuf _ _ rr _) = MkLexBuf 0 [] rr 0
...
p_fail :: Parser t a
p_fail = MkParser (\ buf _ kf _ -> kf buf)

scan :: Parser t t
scan = 
  MkParser (\buf ks kf km ->
    if emptyb buf then 
      km (\toks -> callParser scan (addtoks buf toks) ks kf km)
    else 
...
p <+> q =
  MkParser (\buf ks kf km -> 
    let n = bufpos buf in
    callParser p buf ks (\buf' -> callParser q (rewind n buf') ks kf km) km)

eat :: Eq t => t -> Parser t t
eat x = do y <- scan
           if x == y then return y else p_fail
...
dialogm syntax obey init =
  ...
  where
    arg_loop st ["-e", exp] = 
      do load_file syntax obey st exp; return ()
    arg_loop st (f : args) = 
      do text <- readFile f; 
...
parse (lexer,parser) s =
  case runParser parser (initbuf (lex_string lexer 1 s)) of
    Success e _ -> e
...
readline prompt =
  do 
    ...
    Control.Exception.catch 
      ...
      (\exc -> 
        let _ = exc :: Control.Exception.IOException in return Nothing)
